  ...
  <div>
        ...
        <a-row>
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="所有款项的明细">所有费用</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
            <a-button  @click="do_create()" style="margin:15px 0;margin-left:10px;" type="primary">新建款项</a-button>
    ...
    <a-modal :title="dlg_title" :visible="dlg_new" width="600px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_new=false">
      ...
      <div class="dialog-footer">
        <a-button  @click="do_delete()" style="float:right; margin-left:20px;" v-if="object.state=='create'">取消款项</a-button>
        <!--<a-button  @click="do_delete()" style="float:right; margin-left:20px;" v-if="object.state=='create' && object.id">删除</a-button>-->
        <a-button  @click="do_submit()" style="float:right" v-if="object.state!='done' && object.state!='partdone'" type="primary">提交</a-button>
      </div>
    </a-modal>

    <a-modal title="款项流水" :visible="dlg_trans" width="800px" :footer="null"  @cancel="dlg_trans=false">
    ...
    <a-modal :title="dlg_title_mact" :visible="dlg_mact" width="600px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_mact=false">
      ...
      <div class="dialog-footer">
        <a-popconfirm
              title="终账是部分钱收不回来了, 确定要终账?"
              @confirm="do_recv(true)"
              okText="确定"
              cancelText="取消" v-if="mact_type!='pay'"
            >
              <a-button style="float:right;" type="dashed">终账</a-button>
        </a-popconfirm>

        <a-button  @click="do_pay()" style="float:right" v-if="mact_type=='pay'" type="primary">提交</a-button>
        <a-button  @click="do_recv()" style="float:right;margin-right:10px;" v-if="mact_type!='pay'" type="primary">提交</a-button>
      </div>
    </a-modal>
      ...
      <a-tab-pane tab="明细" key="0">
          <table border="0" cellspacing="0" class="h-tb">
            <thead>
              <tr>
                <td>
                  收入总额: ¥<strong>{{mfmt(sum_data.income_amount)}}</strong><br/>
                </td>
                <td>
                  实收: ¥<strong>{{mfmt(sum_data.income_real)}} </strong><br/>
                  余额: ¥<strong>{{mfmt(sum_data.income_amount- sum_data.income_real)}}</strong>
                </td>
                <td>
                  支出总额: ¥<strong>{{mfmt(sum_data.outcome_amount)}}</strong><br/>
                </td>
                <td>
                  实付: ¥<strong>{{mfmt(sum_data.outcome_real)}}</strong><br/>
                  余额: ¥<strong>{{mfmt(sum_data.outcome_amount- sum_data.outcome_real)}}</strong>
                </td>
                <td>
                  总额毛利: ¥<strong>{{mfmt(sum_data.income_amount-sum_data.outcome_amount)}}</strong><br/>
                </td>
                <td>
                  实额毛利: ¥<strong>{{mfmt(sum_data.income_real-sum_data.outcome_real)}}</strong><br/>
                </td>
              </tr>
            </thead>
          </table>
  ...
  data () {
    return {
      ...
      summary_columns: [
        ...
        {title: '流水实额', key: 'month_real', dataIndex: 'month_real', scopedSlots: { customRender: 'month_real' }}, 
        {title: '欠额', key: 'diff', dataIndex: 'diff', scopedSlots: { customRender: 'diff' }}, 
      ],
  ...
  methods:{
    ...
    print_month_summary(){
      ...
      $('.print').print({
              ...
              deferred: $.Deferred(),
              timeout: 750,
              title: '款项明细汇总 '+this.summary_month,
    ...
    do_create(){
      ...
      this.dlg_title = '创建款项'
      this.dlg_new = true
      this.object = {state: 'create', amount: 0, real: 0, xtype: 'other', come:'income'}
      this.is_edit = false
    },
    ...
    do_recv(is_partdone){
      ...
      env.mact.real = env.mact.real * 1
      if(is_partdone){
        env.mact.is_partdone = true
      }else{
        env.mact.is_partdone = false
      }
      if(env.mact.real >= 0){
