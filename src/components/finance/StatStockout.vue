        ...
        <a-row>
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title=""><router-link :to="{ name: 'StatStockout' }">出库单统计</router-link></a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
          </a-col>
    ...
    <a-form layout="inline" style="padding-left:15px;background-color:rgb(235,235,235);">
      ...
      <a-form-item label="订单类型"  prop="order_type">
        <a-select :defaultValue="summary.order_type" style="width: 160px"  v-model="summary.order_type" >
          ...
          <a-select-option :value="item.value" v-for="item in $store.options.stockout_order_type" v-bind:key="item.value">{{item.label}}</a-select-option>
        </a-select> 
      </a-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      object_columns: [
        ...
        {title: '出库数量', key: 'qty_real', dataIndex: 'qty_real', sorter: (a, b)=>(a.qty_real-b.qty_real)}, 
      ...
      detail_columns:[
        {title: '货品码', key: 'sku', dataIndex: 'sku', }, 
        {title: '名称', key: 'name', dataIndex: 'name',}, 
        {title: '客户', key: 'supplier_code', dataIndex: 'supplier_code', scopedSlots: {customRender:'supplier_code'}},
        {title: '规格', key: 'spec', dataIndex: 'spec'},  
      ...
      options: {},
      time_type_dict: [{value:'create_time', label:'订单创建时间'} , {label:'订单完结时间', value:'finish_time'}, {label:'物料出库时间', value:'out_time'}],
  ...
  methods:{
    load_detail(index, record){
      var env = this
      env.$http.post('/finance/stat/goods/stockout/detail?sku='+record.sku, this.summary).then(function(resp){
    ...
    load_objects(filters, sort){
      ...
      env.$http.post('/finance/stat/goods/stockout', this.summary).then(function(resp){
    ...
    export_objects(){
      ...
      window.open('/finance/stat/goods/stockout?export=xlsx&'+to_querystring(JSON.parse(JSON.stringify(this.summary))), "_blank")
    },
    // end
  },
  // before render
