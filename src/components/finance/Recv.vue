  ...
  <div>
        ...
        <a-row>
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="所有应收款的明细">应收账款</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
            <a-button  @click="do_create()" style="margin:15px 0;margin-left:10px;" type="primary">新建应收款</a-button>
    ...
    <a-modal :title="dlg_title" :visible="dlg_new" width="600px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_new=false">
      ...
      <div class="dialog-footer">
        ...
        <a-button  @click="do_submit()" style="float:right" v-if="object.state!='done' && object.state!='partdone'">提交</a-button>
      </div>
    </a-modal>

    <a-modal title="应收款项流水" :visible="dlg_trans" width="800px" :footer="null"  @cancel="dlg_trans=false">
    ...
    <a-modal :title="dlg_title_mact" :visible="dlg_mact" width="600px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_mact=false">
      ...
      <div class="dialog-footer">
        ...
        <a-button  @click="do_pay()" style="float:right" v-if="mact_type=='pay'">提交</a-button>
        <a-button  @click="do_recv()" style="float:right;margin-right:10px;" v-if="mact_type!='pay'">提交</a-button>
      </div>
    </a-modal>
      ...
      <a-tab-pane tab="明细" key="0">
        <h3 >
          <span>
            ...
            实收: ¥<strong>{{mfmt(sum_data.income_real)}}</strong> |
            未收余额: ¥<strong>{{mfmt(sum_data.income_amount - sum_data.income_real)}}</strong><!-- | 
            支出总额: ¥<strong>{{mfmt(sum_data.outcome_amount)}}</strong> | 
            支出实额: ¥<strong>{{mfmt(sum_data.outcome_real)}}</strong> |
...
export default {
  ...
  data () {
    return {
      ...
      summary_columns: [
        ...
        {title: '实收额', key: 'real', dataIndex: 'real', scopedSlots: { customRender: 'real' }}, 
        {title: '流水实收额', key: 'month_real', dataIndex: 'month_real', scopedSlots: { customRender: 'month_real' }}, 
        {title: '欠收额', key: 'diff', dataIndex: 'diff', scopedSlots: { customRender: 'diff' }}, 
      ],
  ...
  methods:{
    ...
    print_month_summary(){
      ...
      $('.print').print({
              ...
              deferred: $.Deferred(),
              timeout: 750,
              title: '应收款明细汇总 '+this.summary_month,
  ...
  created:function() {
    ...
    // load objects
    this.load_objects([{name:'come',op: '==',val: 'income'}])
  },
  // after render
  mounted: function(){
    this.load_choice()
