        ...
        <a-row>
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="库位管理-一个仓库可能有多个库位">共享仓管理</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
            <a-button  @click="do_create()" style="margin:15px 0;" type="primary">新建共享仓</a-button>
          </a-col>
        </a-row>
    </div>
      ...
      <a-modal :title="dlg_title" :visible="dlg_new" width="600px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_new=false">
        <a-form-model :layout="'horizontal'"   :rules="rules" :model="object">
          <a-form-model-item label="编号" prop="code" :label-col="{span:6}" :wrapper-col="{span:14}">
            <a-input :allowClear="true"  v-model="object.code" ref="code" :disabled="is_edit"></a-input>
          </a-form-model-item>
          <a-form-model-item label="共享公司key" prop="sharekey" :label-col="{span:6}" :wrapper-col="{span:14}">
            <a-input :allowClear="true"  v-model="object.sharekey" :disabled="is_edit"></a-input>
          </a-form-model-item>
          <a-form-model-item label="共享公司仓库编号" prop="sharecode" :label-col="{span:6}" :wrapper-col="{span:14}">
            <a-input :allowClear="true"  v-model="object.sharecode" :disabled="is_edit"></a-input>
          </a-form-model-item>
          <a-form-model-item label="共享仓名称" prop="sharename" :label-col="{span:6}" :wrapper-col="{span:14}">
            <a-input :allowClear="true"  v-model="object.sharename" ></a-input>
          </a-form-model-item>
        </a-form-model>
...
export default {
  ...
  data () {
    return {
      ...
      object_columns: [
        ...
        {title: '共享公司key', key: 'sharekey', dataIndex: 'sharekey'}, 
        {title: '共享公司仓库编号', key: 'sharecode', dataIndex: 'sharecode'}, 
        {title: '共享仓名称', key: 'sharename', dataIndex: 'sharename'}, 
  ...
  methods:{
    ...
    do_submit(){
      ...
      if (this.object.id){
        // update
        this.$http.put('/share/location', this.object).then(function(resp){
        ...
        // create
        this.$http.post('/share/location', this.object).then(function(resp){
          if(resp.data.status=='success'){
            env.load_objects()
            env.dlg_new = false
    ...
    load_objects(filters, sort){
      ...
      env.$http.get('/share/location?q='+JSON.stringify(q)).then(function(resp){
