        ...
        <a-row>
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="工作区管理-一个仓库可能有多个货品">工作区管理</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
            <a-button  @click="do_create()" style="margin:15px 0;" type="primary">新建工作区</a-button>
          </a-col>
        </a-row>
    </div>
...
export default {
  ...
  methods:{
    ...
    do_submit(){
      ...
      if (this.object.id){
        // update
        this.$http.put('/warehouse/workarea', this.object).then(function(resp){
        ...
        // create
        this.$http.post('/warehouse/workarea', this.object).then(function(resp){
          if(resp.data.status=='success'){
            env.load_objects()
            env.dlg_new = false
    ...
    load_objects(filters, sort){
      ...
      env.$http.get('/warehouse/workarea?q='+JSON.stringify(q)).then(function(resp){
