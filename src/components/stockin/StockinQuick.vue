        ...
        <a-row>
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="使用快速入库，分配库位与填写收货数量，确认入库后，直接增加库存记录">快捷入库</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
            <a-button type="primary" @click="new_order()" style="margin:15px 0;">新建入库单</a-button>
    ...
    <div>
        ...
        <a-tabs defaultActiveKey="1">
          <a-tab-pane tab="订单行明细" key="1">
            <div>
              ...
              <a-auto-complete placeholder="供应商" @focus="handle_search_partner()" optionLabelProp="value" :defaultValue="''" @change="handle_search_partner" :backfill="true" v-model="partner_code" @select="select_partner0">
      ...
      <a-modal title="订单行入库" :visible="dlg_putin" width="1000px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_putin=false">
        <a-table bordered :dataSource="lines" :columns="putin_columns" :pagination="false" rowKey="id">
          ...
          <span slot="location" slot-scope="text,record">
            <a-auto-complete placeholder="库位码" @focus="fetch_locations(record)" @select="select_location" optionLabelProp="value" :defaultValue="record.location_code || 'STAGE'" @change="handle_search_location" :backfill="true" v-model="record.location">
              <template slot="dataSource">
                  <a-select-option v-for="item in locations" :key="item.value">
                    <strong>{{item.value}} </strong><span class="certain-search-item-count"> 库存{{ item.qty }}, 总库存{{item.qty_total}}</span>
                  </a-select-option>
        ...
        <a-tabs defaultActiveKey="1" @change="load_order_trans">
          <a-tab-pane tab="订单行明细" key="1">
            <div>
              ...
              <a-auto-complete placeholder="供应商" @focus="handle_search_partner()" optionLabelProp="value" :defaultValue="''" @change="handle_search_partner" :backfill="true" v-model="partner_code" @select="select_partner1">
                <template slot="dataSource">
                    <a-select-option v-for="item in partners" :key="item.code">
                      <strong>{{item.code}} </strong><span class="certain-search-item-count">{{item.name}} {{item.tel}}</span>
                    </a-select-option>
            ...
            <div class="dialog-footer">
              ...
              <a-popconfirm
                ...
                cancelText="取消"
              >
                <a-button v-if="order.state!='cancel' && order.state!='done'" style="float:right;margin-left:10px;" type="primary">取消入库单</a-button>
              </a-popconfirm>
            </div>
          </a-tab-pane>
      ...
      <a-table :columns="object_columns"  :dataSource="objects"  bordered rowKey="id" :pagination="pagination" @change="table_filter_sort" :scroll="{x:'max-content'}">
        ...
        <span slot-scope="text,record,index" slot="action">
            <a-button type="danger" @click="putin_dlg(index, record)" v-if="record.state=='create' || record.state=='part'" style="margin-right:10px">入库</a-button>
    ...
    return {
      ...
      object_columns: [
        ...
        {title: '单号', key: 'order_code', dataIndex: 'order_code',
          scopedSlots: {filterDropdown: 'order_code_filter_dropdown',filterIcon:"order_code_filter_icon", customRender: 'order_code'},
      ...
      new_good_columns: [
        ...
        {title: '库存数量', key: 'qty_able', dataIndex: 'qty_able', scopedSlots: {customRender: 'qty_able'}},
        {title: '入库数量', key: 'qty', dataIndex: 'qty', scopedSlots: {customRender: 'qty'}, width:160,},
        {title: '入库库位', key: 'location_code', dataIndex: 'location_code', scopedSlots: {customRender: 'location_code'}, width:180,},
  ...
  methods:{
    ...
    do_submit_edit(){
      ...
      var data = {lines:this.new_lines, user_code:this.user_code, partner_code: this.partner_code, remark: this.remark, 
          xtype:this.current_order_type, partner_name: this.partner_name}
