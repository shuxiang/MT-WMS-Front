  ...
  <div>
    <div>
        <a-breadcrumb style="padding:20px 20px 20px 0px;">
          ...
          <a-breadcrumb-item title="使用扫码入库，扫取货品条码，填写收货数量，默认收入·STAGE·库位，确认入库后，直接增加库存记录">扫唯一码入库</a-breadcrumb-item>
        </a-breadcrumb>
    </div>
          ...
          <a-tab-pane tab="扫码" key="first" >
            <a-form :layout="'horizontal'">
              <a-form-item label="货品条码" :label-col="{span:4}" :wrapper-col="{span:14}">
                <a-input :allowClear="true" v-model="order_line.barcode" placeholder="必填, 请扫描货品条码" id="c_ol_barcode"  @keyup.enter="enter_barcode"></a-input>
              </a-form-item>
      ...
      <a-table :columns="object_columns"  :dataSource="objects"  bordered rowKey="id" :pagination="pagination" @change="table_filter_sort" :scroll="{x:'max-content'}">
        ...
        <span slot-scope="text,record,index" slot="action">
            <a-button type="danger" @click="scan_dlg_rfid(index, record)"  v-if="record.state=='create' || record.state=='part'" style="margin-right:10px">扫RFID/二维码</a-button>
            <!-- <a-button @click="putin_dlg(index, record)" v-if="record.state=='create' || record.state=='part'" style="margin-right:10px">入库</a-button> -->
            <!--<a-button @click="scan_dlg(index, record)"  v-if="record.state=='create' || record.state=='part'" style="margin-right:10px">扫条码</a-button>-->
