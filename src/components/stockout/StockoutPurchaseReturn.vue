          ...
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="使用自动分配，系统将自动分配可用的库存。
                          ...
                          使用快速拣货，订单将直接扣除库存，但订单需要已经分配完毕才能进行快速拣货。">采购退货单</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
    ...
    <div>
        ...
        <a-tabs defaultActiveKey="1">
          <a-tab-pane tab="订单行明细" key="1">
            <div>
              ...
              <span>  订单类型: </span><a-select :defaultValue="'normal'" style="width: 120px"  v-model="current_order_type" >
      ...
      <a-modal title="明细" :visible="dlg_detail" @cancel="dlg_detail=false" width="1200px" :footer="null">
        ...
        <a-tabs  defaultActiveKey="first">
          <a-tab-pane label="订单行明细" tab="订单行明细" name="first" key="first">
            <div>
              <a-table :dataSource="lines" :columns="line_columns" rowKey="id" bordered :pagination="false">
                <span slot-scope="text,record,index" slot="alloc_line">
                    <a-button  @click="order_line_allocs(index, record)" style="margin-right:20px;" type="primary">分配明细</a-button>
                    <!-- 订单行里有已经发运的就不允许取消了 -->
                    <!-- <a-button  @click="cancel_order_line_allocs(index, record)" v-if="record.qty_ship==0 && record.qty_alloc > 0">取消分配</a-button>-->
                    <a-popconfirm
                      title="此操作将取消该订单行的分配，你确定要取消吗?"
                      @confirm="cancel_order_line_allocs_confirm(index, record)"
                      okText="确定"
                      cancelText="取消"
                    >
                      <a-button v-if="record.qty_ship==0 && record.qty_alloc > 0 && record.qty_pick==0" type="primary">取消分配</a-button>
                    </a-popconfirm>
                </span>
              </a-table>
            </div>
            ...
            <div class="dialog-footer">
              <a-popconfirm
                ...
                cancelText="取消"
              >
                <a-button style="float:right;" v-if="order.state!='cancel' && order.state_ship=='no' && order.state!='done' && order.state_alloc!='no'" type="primary">取消全部分配</a-button>
              </a-popconfirm>
              ...
              <a-popconfirm
                ...
                cancelText="取消"
              >
                <a-button style="float:right;margin-right:20px;" v-if="order.state_alloc!='done' && order.order_type!='produce' && owner_conf.is_enable_mes && !order.has_produce_order" type="primary">创建生产单</a-button>
              </a-popconfirm>
            </div>
          </a-tab-pane>
          <a-tab-pane label="所有已分配未拣货明细" tab="拣货单" name="second" key="second">
            <div>
      ...
      <a-table :columns="order_columns"  :dataSource="objects"  bordered rowKey="id" :pagination="pagination" @change="table_filter_sort" :scroll="{x:'max-content'}">
        ...
        <span slot-scope="text,record" slot="purchase_order_code">
          <span><router-link :to="{ name: 'MesPurchaseOne', params: { order_id: record.purchase_order_code }}">{{ record.purchase_order_code }}</router-link></span>
        </span>
        <span slot-scope="text,record" slot="order_code">
          <span><router-link :to="{ name: 'StockoutOne', params: { order_id: record.id }}">{{ record.order_code }}</router-link></span>
        </span>
        ...
        <span slot-scope="text,record,index" slot="action">
            ...
            <a-popconfirm
              ...
              cancelText="取消"
            >
              <a-button v-if="record.state_alloc!='done'" style="margin-right:10px" type="danger">自动分配</a-button>
            </a-popconfirm>
            ...
            <a-popconfirm
              ...
              cancelText="取消"
            >
              <a-button v-if="record.state_pick!='done' && record.state_alloc=='done' && owner_conf.is_enable_quick_pick" style="margin-right:10px">快速拣货</a-button>
            </a-popconfirm>-->
            <router-link :to="{ name: 'StockoutOne', params: { order_id: record.id }}">
...
export default {
  ...
  data () {
    return {
      ...
      order_columns: [
        ...
        {title: '采购单号', key: 'purchase_order_code', dataIndex: 'purchase_order_code', scopedSlots: { customRender: 'purchase_order_code' }}, 
        {title: '退收金额', key: 'return_real', dataIndex: 'return_real', scopedSlots: { customRender: 'return_real' }}, 
      ...
      flist: [{name: 'order_type', op:'==', val: 'return'}, {name: 'purchase_order_code', op:'!=', val: ''}],
  ...
  methods:{
    ...
    new_goods_class(record, index){
      ...
      if(record.now_price < record.last_out_price){
        return 'red-tb-row'
      }
      return ''
    },
