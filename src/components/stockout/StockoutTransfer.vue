  ...
  <div>
          ...
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="使用自动分配，系统将自动分配可用的库存。
                          ...
                          使用快速拣货，订单将直接扣除库存，但订单需要已经分配完毕才能进行快速拣货。">调拨出库单</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
            ...
            <!--<a-button @click="do_create_merge()" type="primary" style="margin:15px 0;margin-left:20px;" v-if="owner_conf.is_enable_stockout_merge">创建出库波次</a-button>-->
          </a-col>
        </a-row>
    </div>
    ...
    <div>
        ...
        <a-tabs defaultActiveKey="1">
          <a-tab-pane tab="订单行明细" key="1">
            <div>
              ...
              <span>  公司: </span>
              <a-auto-complete placeholder="公司" @focus="handle_search_company()" optionLabelProp="value" :defaultValue="''" @change="handle_search_company" @select="select_company" :backfill="true" v-model="order_new.transfer_in_company_code">
              ...
              <span>  仓库: </span>
              <a-auto-complete placeholder="仓库" @focus="handle_search_warehouse()" optionLabelProp="value" :defaultValue="''" @change="handle_search_warehouse" @select="select_warehouse" :backfill="true" v-model="order_new.transfer_in_warehouse_code">
                <template slot="dataSource">
              ...
              <span>  货主: </span>
              <a-auto-complete placeholder="货主" @focus="handle_search_owner()" optionLabelProp="value" :defaultValue="''" @change="handle_search_owner" @select="select_owner" :backfill="true" v-model="order_new.transfer_in_owner_code">
                <template slot="dataSource">
        ...
        <a-tabs  defaultActiveKey="1">
          <a-tab-pane tab="订单行明细" key="1">
            <div>
              ...
              <!--
              <span>  公司: </span>
              <a-auto-complete placeholder="公司" @focus="handle_search_company()" optionLabelProp="value" :defaultValue="''" @change="handle_search_company" @select="select_company" :backfill="true" v-model="order_new.transfer_in_company_code" :disable="true">
                <template slot="dataSource">
                    <a-select-option v-for="item in companys" :key="item.code">
                      <strong>{{item.code}} </strong><span class="certain-search-item-count">{{item.name}} {{item.tel}}</span>
                    </a-select-option>
              ...
              <span>  仓库: </span>
              <a-auto-complete placeholder="仓库" @focus="handle_search_warehouse()" optionLabelProp="value" :defaultValue="''" @change="handle_search_warehouse" @select="select_warehouse" :backfill="true" v-model="order_new.transfer_in_warehouse_code" :disable="true">
                <template slot="dataSource">
                    <a-select-option v-for="item in warehouses" :key="item.code">
                      <strong>{{item.code}} </strong><span class="certain-search-item-count">{{item.name}} {{item.tel}}</span>
                    </a-select-option>
              ...
              <span>  货主: </span>
              <a-auto-complete placeholder="货主" @focus="handle_search_owner()" optionLabelProp="value" :defaultValue="''" @change="handle_search_owner" @select="select_owner" :backfill="true" v-model="order_new.transfer_in_owner_code" :disable="true">
                <template slot="dataSource">
                    <a-select-option v-for="item in owners" :key="item.code">
                      <strong>{{item.code}} </strong><span class="certain-search-item-count">{{item.name}} {{item.tel}}</span>
                    </a-select-option>
            ...
            <div class="dialog-footer">
              ...
              <a-popconfirm
                ...
                cancelText="取消"
              >
                <a-button v-if="order.state!='cancel' && order.state!='done' && order.state_alloc=='no'" style="float:right;margin-right:20px;" type="primary">取消</a-button>
              </a-popconfirm>
              ...
              <a-popconfirm
                ...
                cancelText="取消"
              >
                <a-button type="primary" style="float:right;margin-right:20px;" v-if="order.state_alloc!='done' && (order.order_type=='sale' || order.order_type=='normal') && owner_conf.is_enable_mes && !order.has_produce_order && order.state!='cancel'">创建生产单</a-button>
              </a-popconfirm>
              <a-popconfirm
                title="确定要修改订单吗?"
      ...
      <a-modal title="订单行出库-指定库位库存分配" :visible="dlg_alloc" width="1200px" :footer="null" :keyboard="false" :maskClosable="false" @cancel="dlg_alloc=false">
        ...
        <div class="dialog-footer">
          <a-button  @click="do_alloc_lines()" style="float:right">确定分配</a-button>
        </div>
      </a-modal>
      ...
      <a-form layout="inline" v-if="owner_conf.is_enable_search_bar" style="padding-left:15px;background-color:rgb(235,235,235);">
        ...
        <a-form-item label="公司">
          <a-auto-complete placeholder="公司" @focus="handle_search_company()" optionLabelProp="value" :defaultValue="''" @change="handle_search_company" @select="select_company2" :backfill="true" v-model="query.transfer_in_company_name" :allowClear="true">
            <template slot="dataSource">
        ...
        <a-form-item label="仓库">
          <a-auto-complete placeholder="仓库" @focus="handle_search_warehouse2()" optionLabelProp="value" :defaultValue="''" @change="handle_search_warehouse2" :backfill="true" v-model="query.transfer_in_warehouse_name" :allowClear="true">
            <template slot="dataSource">
        ...
        <a-form-item label="货主">
          <a-auto-complete placeholder="货主" @focus="handle_search_owner2()" optionLabelProp="value" :defaultValue="''" @change="handle_search_owner2" :backfill="true" v-model="query.transfer_in_owner_name" :allowClear="true">
            <template slot="dataSource">
      ...
      <a-table :columns="order_columns"  :dataSource="objects"  bordered rowKey="id" :pagination="pagination" @change="table_filter_sort">
        <span slot-scope="text,record" slot="id">
          <!--
          <a-checkbox @change="add2merge" v-if="owner_conf.is_enable_stockout_merge && record.state_alloc=='no' && record.state=='create' && (!record.merge_order_code)" :value="record.order_code"></a-checkbox>
          -->
          <span>{{ record.id }}</span>
        </span>
        ...
        <span slot-scope="text,record" slot="sub_order_type">
          <span>{{ translate('sub_out_type', record.sub_order_type) }}</span>
        </span>
        ...
        <span slot-scope="text,record,index" slot="action"><!--
            ...
            <a-popconfirm
              ...
              cancelText="取消"
            >
              <a-button v-if="record.state_pick!='done' && record.state_alloc=='done' && $store.owner_conf.is_enable_quick_pick" style="margin-right:10px">快速拣货</a-button>
            </a-popconfirm>-->
            <router-link :to="{ name: 'StockoutOne', params: { order_id: record.id }}">
...
export default {
  ...
  data () {
    return {
      ...
      order_columns: [
        {title: 'ID', key: 'id', dataIndex: 'id', sorter: (a, b)=>(a.id-b.id), scopedSlots: { customRender: 'id' }}, 
        ...
        // {title: '类型', key: 'order_type', dataIndex: 'order_type', scopedSlots: { customRender: 'order_type' }, filters:this.$store.options['stockout_order_type']},
        // {title: '子类型', key: 'sub_order_type', dataIndex: 'sub_order_type', scopedSlots: { customRender: 'sub_order_type' }, filters:this.$store.options['sub_out_type']},
        {title: '状态', key: 'state', dataIndex: 'state', scopedSlots: { customRender: 'state' }, },
        {title: '目标公司', key: 'transfer_in_company_name', dataIndex: 'transfer_in_company_name',},
        {title: '目标仓库', key: 'transfer_in_warehouse_name', dataIndex: 'transfer_in_warehouse_name',},
        {title: '目标货主', key: 'transfer_in_owner_name', dataIndex: 'transfer_in_owner_name',},
      ...
      // new order
      order_new: {receiver_info:{}, sender_info: {}, order_type:'transfer'},
      dlg_new: false,
      new_goods:[],
      new_good_columns: [
        {title: '货品码', key: 'sku', dataIndex: 'sku'}, 
        ...
        {title: '库存数', key: 'qty_able', dataIndex: 'qty_able', scopedSlots: {customRender: 'qty_able'}},
  ...
  methods:{
    ...
    new_order(){
      this.dlg_new = true
      this.order_new = {receiver_info:{}, sender_info: {}, order_type:'transfer'}
      this.current_good = ''
      this.new_goods = []
      this.new_lines = []
      this.new_lines_ids = []
    ...
    do_submit(){
      ...
      this.order_new.order_type = 'transfer'
    ...
    search_good(){
      ...
      //if(this.current_good){
        this.$http.get('/inv/good?with_inv_qty=true&qtype=out&q='+this.current_good).then(function(resp){
    ...
    add2merge(e){
      ...
      var idx = this.merge_list.indexOf(val)
      var checked = e.target.checked
      if(checked && idx >-1){
        //
      }else if(checked){
        this.merge_list.push(val)
      }else if((!checked) && idx >-1){
        this.merge_list.splice(idx, 1)
      }else{
        //
      }
    },
    ...
    do_create_merge(){
      var env = this
      if(this.merge_list.length>1){
        this.$http.post('/stockout/merge', {lines:this.merge_list}).then(function(resp){
          if(resp.data.status=='success'){
            env.$message.success('创建成功, 波次号: '+resp.data.order_code, 3)
            env.search()
          }else{
        ...
        env.$message.warning('请勾选需要波次的单据, 数量大于1')
      }
    },
    ...
    cancel_order(){
      ...
      this.$http.post('/stockout/cancel/'+this.order.id).then(function(resp){
        if(resp.data.status=='success'){
          ...
          env.load_objects([{name:'order_type', op:'eq', val:'transfer'}])
        }else{
    ...
    do_submit_edit(){
      var env = this
      // load order lines
      var data = {lines:this.new_lines, user_code:this.user_code, partner_code: this.partner_code, remark: this.remark, order_type:this.current_order_type}
      data.transfer_in_company_code = this.order_new.transfer_in_company_code
      data.transfer_in_company_name = this.order_new.transfer_in_company_name
      data.transfer_in_warehouse_name = this.order_new.transfer_in_warehouse_name
      data.transfer_in_warehouse_code = this.order_new.transfer_in_warehouse_code
      data.transfer_in_owner_name = this.order_new.transfer_in_owner_name
      data.transfer_in_owner_code = this.order_new.transfer_in_owner_code
    ...
    select_company(val, option){
      ...
      env.companys.forEach(function(v,i){
        if(v.code==val){
          env.order_new.transfer_in_company_name = v.name
          return false
        }
      })
    },
    select_company2(val, option){
      ...
      env.companys.forEach(function(v,i){
        if(v.name==val){
          env.query.transfer_in_company_code = v.code
          return false
        }
      })
    },
    select_warehouse(val, option){
      ...
      env.warehouses.forEach(function(v,i){
        if(v.code==val){
          env.order_new.transfer_in_warehouse_name = v.name
          return false
        }
      })
    },
    select_owner(val, option){
      ...
      env.owners.forEach(function(v,i){
        if(v.code==val){
          env.order_new.transfer_in_owner_name = v.name
          return false
        }
      })
    },
    handle_search_warehouse(val){
      ...
      this.$http.get('/warehouse/warehouse?company_code='+env.order_new.transfer_in_company_code+'&q='+JSON.stringify(q)).then(function(resp){
    ...
    handle_search_owner(val){
      ...
      this.$http.get('/warehouse/partner/owner?company_code='+env.order_new.transfer_in_company_code+'&q='+JSON.stringify(q)).then(function(resp){
    ...
    handle_search_warehouse2(val){
      ...
      this.$http.get('/warehouse/warehouse?company_code='+env.query.transfer_in_company_code+'&q='+JSON.stringify(q)).then(function(resp){
          env.warehouses = resp.data.objects
      })
    },
    handle_search_owner2(val){
      ...
      this.$http.get('/warehouse/partner/owner?company_code='+env.query.transfer_in_company_code+'&q='+JSON.stringify(q)).then(function(resp){
          env.owners = resp.data.objects
      })
    },
    //end
  ...
  created:function() {
    var env = this
    // load objects
    this.load_objects([{name:'order_type', op:'eq', val:'transfer'}])
  },
}
</script>
