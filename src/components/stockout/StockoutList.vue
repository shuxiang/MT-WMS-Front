          ...
          <a-col :span="6">
            <a-breadcrumb style="padding:20px 20px 20px 0px;">
              ...
              <a-breadcrumb-item title="使用自动分配，系统将自动分配可用的库存。
                          ...
                          使用快速拣货，订单将直接扣除库存，但订单需要已经分配完毕才能进行快速拣货。">出库单</a-breadcrumb-item>
            </a-breadcrumb>
          </a-col>
          <a-col :span="18">
    ...
    <div>
      ...
      <a-form layout="inline" v-if="owner_conf.is_enable_search_bar" style="padding-left:15px;background-color:rgb(235,235,235);">
        ...
        <a-form-item label="客户">
          <a-auto-complete placeholder="客户" @focus="handle_search_partner()" optionLabelProp="value" :defaultValue="''" @change="handle_search_partner" :backfill="true" v-model="query.partner_code" :allowClear="true">
            <template slot="dataSource">
                <a-select-option v-for="item in partners" :key="item.code">
                  <strong>{{item.code}} </strong><span class="certain-search-item-count">{{item.name}} {{item.tel}}</span>
                </a-select-option>
        ...
        <a-form-item label="子类型">
          <a-select :defaultValue="query.sub_order_type"  v-model="query.sub_order_type" style="width:120px;" :allowClear="true">
            <a-select-option value="" v-bind:key="''">--所有--</a-select-option>
            <a-select-option :value="item.value" v-for="item in options.sub_out_type" v-bind:key="item.value">{{item.label}}</a-select-option>
          </a-select> 
        </a-form-item>
  ...
  data () {
    return {
      ...
      order_columns: [
        ...
        {title: '子类型', key: 'sub_order_type', dataIndex: 'sub_order_type', scopedSlots: { customRender: 'sub_order_type' }, },
        {title: '状态', key: 'state', dataIndex: 'state', scopedSlots: { customRender: 'state' }, },
        {title: '客户', key: 'partner_code', dataIndex: 'partner_code', 
